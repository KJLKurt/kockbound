...
export function stepPods(w:World,commands:readonly Input[],emit:Emit){
 ...
 for(const p of w.players){const command=commands.find(c=>c.participantId===p.id),item=p.heldItem;if(!p.alive||item?.kind!=='pod')continue;
  if(now>=item.expiresAt||(command?.useItem&&!command.dropItem&&now>=(p.stunnedUntil??0))){const {dx,dz}=itemDirection(p,now>=item.expiresAt?undefined:command);const x=p.x+dx*.8,z=p.z+dz*.8;ground.push({...item,x,z,vx:0,vz:0,thrown:false,owner:p.id,blockedId:null,blockedUntil:0,armedAt:now+15,expiresAt:now+215});p.heldItem=null;emit(w,'podarm',x,z,p.id);}
 }
