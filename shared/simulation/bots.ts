...
export function botInputs(w: World): Input[] {
  ...
  for (const p of w.players) {
    ...
    if (w.tick >= mem.nextTick) {
      ...
      if(danger){mem.x=p.x-danger.x;mem.z=p.z-danger.z;dash=false;}else if(loot&&distanceToEdge>2.2){mem.x=loot.x-p.x;mem.z=loot.z-p.z;dash=loot.kind==='crate'&&Math.hypot(mem.x,mem.z)<3;}
