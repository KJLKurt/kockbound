...
/** Same sampled targeting for the authoritative action and its local preview. */
export function removerTarget(w:World,origin:{x:number;z:number},direction:{dx:number;dz:number}){
 for(const distance of [3,4,5,6]){const x=origin.x+direction.dx*distance,z=origin.z+direction.dz*distance,id=tileAt(w,x,z);if(id&&id!=='core'&&!w.tiles?.[id])return {id,x,z};}
 return null;
}
